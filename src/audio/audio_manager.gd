...
func play_one_shot_attached_with_params(_guid: String, node: Node3D, params: Dictionary) -> void:
	if !FmodServer.check_event_guid(_guid):
		print("No event was found with guid: " + _guid)
		return
	
	FmodServer.play_one_shot_using_guid_attached_with_params(_guid, node, params)
...
func set_ui_volume(volume: float) -> void:
	if ui_bus == null:
		ui_bus = FmodServer.get_bus_from_guid("")
	if music_bus != null:
		ui_bus.set_volume(volume)
