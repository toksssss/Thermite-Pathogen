...
func _ready() -> void:
	if GameManager.instance.ui_counter == 0: 
		...
		is_disabled = true
	GameManager.instance.ui_opened.connect(func() -> void: is_disabled = true, CONNECT_DEFERRED)
	GameManager.instance.ui_closed.connect(func() -> void: is_disabled = false, CONNECT_DEFERRED)
